...
nai = new function () {
  ...
  lib.log = function (message, group) {
    if (this.DEBUG) {
      if (group) { console.log(group, message) } else { console.log(message) }
    }
  }
  ...
  lib.handleResponse = function (onErrorResponse, onNoResponse, onUnexpected) {
    return function (error) {
      if (error.response) {
        ...
        if (onErrorResponse) { onErrorResponse(error.response) } else {
        ...
        if (onNoResponse) { onNoResponse(error.request) } else { console.log(error.request) }
      } else {
        // Something happened in setting up the request that triggered an Error
        if (onUnexpected) { onUnexpected(error.message) } else { console.log('Error', error.message) }
      }
